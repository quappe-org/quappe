...
- **Semantic search** with a multilingual e5 embedding model (Xenova/transformers).
  Fulltext + vector similarity, with auto-category suggestion on new theses.
- **Reflection reports** generated by a local LLM (Ollama):
  - `/my` — "Mein Standpunkt", a private on-demand reflection over your own
    theses, arguments and votes. No blaming, only reference.
  - `/pulse` — "Community Puls", a daily-refreshed public overview: hot
    theses, complex threads, driving categories. Falls back to raw stats if
