...
- **Known dependency advisories (assessed, accepted for MVP).** `npm audit` / Dependabot flags 3 issues (protobufjs critical, onnx, sharp) — all transitive via `@xenova/transformers` (server-side embedding model). The only `npm audit fix` is a downgrade to `transformers@1.4.2`, a breaking change that would kill semantic search. The flagged vulns concern parsing *untrusted* protobuf/ONNX; we only load a fixed, trusted HuggingFace model — no user input flows through protobuf, so real exposure is ~nil. Proper fix later: migrate to the maintained successor `@huggingface/transformers`. Do not "fix" this with `--force`.
